...
**As of**: phase2-build2 **merged to `main` via PR #27** (merge commit `a2d132c`, feature work `1146837`). Doc-only follow-ups: **PR #28** (`f2e03a8`) JSDoc fix + post-review note; **PR #29** (`6ed0599`) the build3 brief + log entry; **PR #30** the build-state SHA bump. **phase2-build3 then SHIPPED via PR #31** (`f2be8dd`) — the three standalone slices (#7 demo/clear, #8 plan-capacity bar, #9 return-to-backlog), no model/schema/reducer/server-op change; **PR #32** (`cf2a800`) the dev WSL bind fix (dev-tooling only). **`main` now at `cf2a800`**, `origin == local`. Tree: **191 unit/integration tests green** (`npm test`, which runs the theme-drift gate *first*), `tsc --noEmit` clean, theme-drift `ok: /var/www/plan`. NOTE: this snapshot's architecture sections predate build3 — build3's additions (board-selectors `planCapacity`/`planBarData`, the capacity bar, demo/clear, the on-card return control, `confirmModal`) are not yet folded into the prose below; author `build-state-after-phase2-build3.md` if a full refresh is wanted.
...
per-build SHA references stay valid. **phase2-build2 merged the same way (PR #27, `a2d132c`)**, followed by doc-only PRs #28 (`f2e03a8`), #29 (`6ed0599`), #30, then **phase2-build3 (PR #31, `f2be8dd`)** and the dev-bind fix (PR #32, `cf2a800`).
